...
export default function StudyCard() {
  ...
  {items.map((item, index) => (
    <div key={index}>
      <span>{item}</span>
      <IconButton style={{  color: "green",float:'right',padding:0 }}><EditIcon sx={{my:10}} onClick={() => handleOpen(index)} /></IconButton>
        
      <IconButton style={{  color: "red",float:'right',padding:0 }} variant="contained"  ><DeleteIcon  onClick={() => handleDeleteItem(index)}/></IconButton>
    </div>
  ))}
  <Dialog  sx={{minWidth:300}} open={open} onClose={handleClose}>
    <DialogTitle>{editItem !== null ? 'Edit ' : 'work'}</DialogTitle>
    <DialogContent>
      <TextField autoFocus fullWidth label="work" value={inputValue} onChange={(e) => setInputValue(e.target.value)} />
    </DialogContent>
